...
######################################################################
#
# OldUTF8 - "Yet Another JPerl" Source code filter to escape old UTF-8
#
...
=head1 SYNOPSIS
  ...
  C:\>perl OldUTF8.pm old UTF-8_script.pl > Escaped_script.pl.e
...
Also \b and \B are redefined as follows to backward compatibility.
  ...
  escape        class
  ---------------------------------------------------------------------------
  \b            (?:(?:\A|(?<=(?:[\x00-\x7F]|[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF4][\x80-\xBF]{3})[\x00-\xFF])|(?<=[^0-9A-Z_a-z]))(?=[0-9A-Z_a-z])|(?<=[0-9A-Z_a-z])(?=(?:[\x00-\x7F]|[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF4][\x80-\xBF]{3})[\x00-\xFF]|[^0-9A-Z_a-z]|\z))
  \B            (?:(?<=[0-9A-Z_a-z])(?=[0-9A-Z_a-z])|(?:(?<=(?:[\x00-\x7F]|[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF4][\x80-\xBF]{3})[\x00-\xFF])|(?<=[^0-9A-Z_a-z]))(?=(?:[\x00-\x7F]|[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF4][\x80-\xBF]{3})[\x00-\xFF]|[^0-9A-Z_a-z]))
  ---------------------------------------------------------------------------
...
=item * chop --> Eoldutf8::chop

=item * split --> Eoldutf8::split

=item * length

=item * substr

=item * index --> Eoldutf8::index

=item * rindex --> Eoldutf8::rindex
...
=item * chr --> Eoldutf8::chr or Eoldutf8::chr_

multiple octet code can also be handled.

=item * glob --> Eoldutf8::glob or Eoldutf8::glob_

  @glob = Eoldutf8::glob($string);
  @glob = Eoldutf8::glob_;

Eoldutf8::glob provides a portable enhanced DOS-like globbing for the OldUTF8 software.
Eoldutf8::glob lets you use wildcards in directory paths, is case-insensitive, and
...
=item * ord --> OldUTF8::ord or OldUTF8::ord_

multiple octet code can also be handled when "use OldUTF8 qw(ord);".
It means not compatible with JPerl.

=item * reverse --> OldUTF8::reverse

multiple octet code can also be handled in scalar context when "use OldUTF8 qw(reverse);".
It means not compatible with JPerl.
